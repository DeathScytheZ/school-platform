...
export const load = async ({ fetch, locals }) => {
    ...
    try {
        const [studentsResponse, performanceResponse, behaviorResponse, gradesResponse] = await Promise.all([
            fetch(`http://localhost:3000/api/teacher/students/${accountId}`),
            fetch(`http://localhost:3000/api/teacher/performance/${accountId}`),
            fetch(`http://localhost:3000/api/teacher/behavior/${accountId}`),
            fetch(`http://localhost:3000/api/teacher/grades/${accountId}`)
        ]);
